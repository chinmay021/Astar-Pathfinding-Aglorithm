...
def algorithm(draw, grid, start, end):
    ...
    f_score[start] = heuristic(start.get_pos(), end.get_pos()) + g_score[start]

    # set to check which all nodes are there in open set
    open_set_hash = {start}
    ...
    while not open_set.empty():
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
        # dequeue highest priority queue i.e lowest value f_score
        current = open_set.get()[2]
        open_set_hash.remove(current)  # also remove it form the set
